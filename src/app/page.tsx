...
export const metadata: Metadata = {
  ...
  description: "AI-powered CV feedback built specifically for Malaysian employers, GLCs, MNCs and government roles. Free first analysis, no signup required.",
};
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden">
          <div aria-hidden className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_top,_var(--secondary)_0%,_transparent_70%)]" />
          <div className="mx-auto max-w-5xl px-4 py-20 text-center sm:px-6 sm:py-28">
            ...
            <h1 className="text-[24px] font-extrabold leading-[1.15] tracking-tight text-primary sm:text-4xl">
              Get Your CV Ready for <span className="whitespace-nowrap">Malaysia&apos;s Job Market</span>
            </h1>
            ...
            <div className="mt-10 flex flex-col items-center gap-3">
              <Button asChild variant="navy" size="xl">
                <Link href="/analyze">Analyze My CV Free <ArrowRight className="ml-1" /></Link>
              </Button>
              <p className="text-sm text-muted-foreground">No signup required for first analysis</p>
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-[22px] font-bold tracking-tight text-primary sm:text-4xl">Everything You Need to Land the Job</h2>
              <p className="mt-3 text-[15px] text-muted-foreground">One platform. Every step of your Malaysian job search.</p>
            </div>
                ...
                const Card = (
                  <div key={f.title} className="rounded-2xl border border-border bg-card p-6 shadow-sm transition hover:shadow-md hover:-translate-y-0.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {f.comingSoon && <span className="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">Coming soon</span>}
                    </div>
                    <p className="mt-2 text-[15px] text-muted-foreground">{f.desc}</p>
                  </div>
                );
                return f.href ? <Link key={f.title} href={f.href} className="block">{Card}</Link> : <div key={f.title}>{Card}</div>;
              })}
            </div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="bg-secondary/40 py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-[22px] font-bold tracking-tight text-primary sm:text-4xl">How It Works</h2>
              <p className="mt-3 text-[15px] text-muted-foreground">Three steps. Thirty seconds. Real Malaysian context.</p>
            </div>
            <div className="mt-12 grid gap-6 md:grid-cols-3">
              {[
                ...
                <div key={s.title} className="relative rounded-2xl border border-border bg-card p-6 shadow-sm">
                  <div className="absolute -top-3 left-6 rounded-full bg-primary px-3 py-0.5 text-xs font-bold text-primary-foreground">Step {i + 1}</div>
            ...
            <div className="mx-auto mt-20 max-w-2xl text-center">
              ...
              <h3 className="text-[22px] font-bold tracking-tight text-primary sm:text-3xl">Review Your LinkedIn Profile</h3>
              <p className="mt-3 text-[15px] text-muted-foreground">AI feedback on your headline, about section, experience, skills and Malaysian market fit.</p>
            </div>
            ...
            <div className="mt-8 flex justify-center">
              <Button asChild variant="navy" size="lg">
                <Link href="/linkedin-review">Review My LinkedIn <ArrowRight className="ml-1" /></Link>
              </Button>
            </div>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-[22px] font-bold tracking-tight text-primary sm:text-4xl">What Makes ResuMY Different</h2>
              <p className="mt-3 text-[15px] text-muted-foreground">Generic CV tools miss Malaysia. We don&apos;t.</p>
            </div>
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { icon: MapPin, title: "Knows Your Target Employer", desc: "A Petronas CV and a startup CV aren't the same document. We score yours against the exact employer type you're targeting." },
                { icon: Globe2, title: "BM / English Balance", desc: "Government roles may expect Bahasa. MNCs want sharp English. We flag where your language choice wins points." },
                { icon: Search, title: "Beats the Local ATS", desc: "We pull keywords from real Malaysian listings, so your CV clears the filters local recruiters actually use." },
                { icon: GraduationCap, title: "Fresh Grad Mode", desc: "Overseas tools tell you to drop your photo and CGPA. Malaysian employers often expect both." },
              ].map((f) => (
                <div key={f.title} className="rounded-2xl border border-border bg-card p-6 shadow-sm transition hover:shadow-md hover:-translate-y-0.5">
                  <div className="mb-4 inline-flex size-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><f.icon className="size-5" /></div>
                  <h3 className="text-base font-semibold text-foreground">{f.title}</h3>
                  <p className="mt-2 text-[15px] text-muted-foreground">{f.desc}</p>
                </div>
              ))}
        ...
        <section className="relative overflow-hidden bg-primary py-16 text-primary-foreground">
          <div className="mx-auto flex max-w-4xl flex-col items-center px-4 text-center sm:px-6">
            <h2 className="text-[22px] font-bold sm:text-4xl">Ready to stand out to Malaysian employers?</h2>
            <p className="mt-3 max-w-xl text-[15px] text-primary-foreground/80">First analysis is free. No signup. No credit card.</p>
            <Button asChild variant="navy" size="xl" className="mt-8 bg-background text-primary hover:bg-background/90">
              <Link href="/analyze">Analyze My CV Free</Link>
            </Button>
          </div>
        </section>
      </main>
