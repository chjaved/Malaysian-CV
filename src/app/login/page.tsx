...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (err) {
      ...
      if (msg.includes("email not confirmed") || msg.includes("not confirmed")) setError("Please confirm your email address before logging in.");
      ...
      <main className="flex flex-1 items-center justify-center px-4 py-12">
        <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            New here? <Link href="/signup" className="font-medium text-primary hover:underline">Sign up</Link>
          </p>
        </div>
      </main>
      <SiteFooter />
