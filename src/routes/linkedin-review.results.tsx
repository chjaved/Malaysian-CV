...
export const Route = createFileRoute("/linkedin-review/results")({
  ...
  head: () => ({
    meta: [
      { title: "Your LinkedIn Review — ResuMY" },
      {
        name: "description",
...
function LinkedInResultsPage() {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem("resumy:lastLinkedIn");
    if (!raw) {
...
export function LinkedInResultsView({ result, meta }: Stored) {
  const shareText = encodeURIComponent(
    `Just got my LinkedIn profile reviewed for Malaysia's job market with ResuMY 🇲🇾`,
  );
    ...
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2">
            <Button asChild variant="navy" size="xl">
              <Link to="/linkedin-review">Review Another Profile</Link>
            </Button>
          ...
          <div className="mt-10 rounded-2xl border border-border bg-secondary/40 p-6 text-center">
            <p className="flex items-center justify-center gap-2 text-sm font-medium text-foreground">
              <Share2 className="size-4" /> Share ResuMY with a friend who needs this
            </p>
