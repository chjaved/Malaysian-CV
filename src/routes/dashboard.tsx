          ...
          <>
            ...
            {error && (
              <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                Couldn't load your analyses: {error}
              </div>
            )}
            ...
            {!error && rows && rows.length === 0 && (
              <div className="rounded-2xl border border-border bg-card p-10 text-center">
                ...
                <Link to="/analyze" className="mt-6 inline-flex rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-sm hover:bg-primary/90">
                  Analyze your CV
                </Link>
              </div>
            )}
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 text-sm font-medium text-foreground">
                            <span className="capitalize">{r.industry}</span>
                            <span className="text-muted-foreground">·</span>
                            <span className="capitalize">{r.company_type}</span>
                            <span className="text-muted-foreground">·</span>
                            <span className="capitalize">{r.experience_level}</span>
                          </div>
                          <div className="mt-1 text-xs text-muted-foreground">{formatDate(r.created_at)}</div>
                        </div>
                        <div className={`shrink-0 inline-flex items-center justify-center rounded-full border px-3 py-1 text-sm font-semibold ${scoreColor(r.overall_score)}`}>
                          {r.overall_score}
                        </div>
            ...
            <div key={s.key} className="rounded-lg border border-border p-3">
              <div className="flex items-center justify-between">
                ...
                {typeof v.score === "number" && (
                  <span className={`rounded-full border px-2 py-0.5 text-xs font-medium ${scoreColor(v.score)}`}>{v.score}</span>
                )}
              </div>
  ...
  return (
    <div className="mt-2">
      ...
      <div className="mt-1 flex flex-wrap gap-1.5">
        {items.map((k, i) => (
          <span key={i} className={`inline-flex rounded-md border px-2 py-0.5 text-xs ${cls}`}>{k}</span>
        ))}
      </div>
    </div>
  );
